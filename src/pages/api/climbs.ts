...
/**
 * S-04 climb-save endpoint.
 *
 * Single authenticated JSON mutation for the inline route-row form
 * (`src/components/climbs/ClimbLogForm.tsx`). React islands MUST go through
 * this endpoint instead of importing `@/lib/private-state` directly —
 * that module carries the Supabase session cookies and Strapi API token
 * and is server-only by contract (see `src/lib/private-state/index.ts`).
 *
 * Contract
 *
 * - Body: `{ routeId: string; climbedOn: string; note?: string | null }`
...
function resolveErrorMessage(code: ApiErrorCode): string {
  ...
  switch (code) {
    ...
    // `not_found` / `duplicate_project` cannot be produced by `createClimb`;
    // they're handled here for completeness so future endpoints that share
    // the helper get the same Polish phrasing fallback.
    case "not_found":
